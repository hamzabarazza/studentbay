...
public class AddShopView extends VerticalLayout implements View{
    ...
    public void setUp(){
       ...
       final User user = (User) session.getAttribute(Roles.CURRENT_USER);
        if(user.getLogged()){
            ...
            try {
                if(MeinShopControl.getMeinShop(user.getID()).equals("")){
                    ...
                    Label fehler = new Label("Sie haben bereits einen Shop erstellt!");
                    addShopPanel.addComponent(fehler);
                }
